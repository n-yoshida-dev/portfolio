...
- [x] Vercel に接続して本番 URL を得る（ユーザー操作）
  完了条件：本番 URL で `/llms.txt` が開き、リンクが本番 URL になっている（2026-09-26 確認。URL は `README.md` 冒頭）
- [x] 本番 URL で Ask AI のプロンプトを実際に ChatGPT / Claude に投げ、回答が Skills の区分を守っているか確認する
  完了条件：回答が「商用実務」と「個人開発」を混同していない。混同するなら llms.txt の指示文を直す（2026-09-26 Claude で確認。混同なし。ChatGPT は未確認）
- [x] Ask AI のプロンプトに「日本語で答える」を足す（回答が英語で返ってきたため）
  完了条件：本番の `/ask` のプロンプト末尾に `Answer in Japanese.` があり、Claude に投げた回答が日本語で返る（2026-09-26 本番で確認。区分も維持）
...
- [ ] GitHub の bio「新人エンジニア」がサイトの「約 10 年」と食い違う（2026-09-26 に Claude の回答が指摘）。直すか、直すなら文案が要るか（変更は GitHub の設定画面でユーザーが行う）
