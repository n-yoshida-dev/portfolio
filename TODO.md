...
2026-09-26 にユーザーが「文章が多すぎて見る気にならない」と判断。自動化（保留節）より先にやる。
参考にした個人サイト 23 件の共通パターンは `logs/decisions.md` 2026-09-26。

- [x] Home を 1 画面にする（profile.md の本文を Home から外し、`highlights` 3 行 + 一言 + 入口 3 つ + 主なプロジェクト 3 件 + 仕組みの題名だけにする）
  完了条件：PC（1280×800）と 375px 幅で、スクロールせずに「何の人か（要点 3 行）」と入口 3 つが見える。`/profile.md` と `/llms-full.txt` には本文が残っている（2026-09-26 ローカルのプレビューで撮影して確認）
- [ ] Projects / Systems の詳細ページを「要約 3 行 + 折りたたみ」にする（本文は `<details>` で畳む。Markdown 版は全文のまま）
  完了条件：各詳細ページを開いた直後に見える文字数が 800 字以下。`/projects/<slug>.md` の内容は変わらない
- [ ] Skills と Journey の表を短くする（1 セル 1 行・体言止め。「読み方」の表は 1 行の注記に。根拠は「ADR 36 本」のように数字で）
  完了条件：表のセルに句点（。）が無い。区分（商用実務 / 個人開発 / 理解確認済み / 学習中）は保たれている
